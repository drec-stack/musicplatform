(function() {
    ...
    UIManager.prototype.setPage = function() {
        ...
        h+='</div></div><div class="settings-section"><h2 class="settings-section-title">Data</h2><div class="data-actions"><button class="btn btn-secondary" id="exportDataBtn">Export JSON</button><button class="btn btn-danger" id="clearDataBtn">Clear all</button></div></div></div>';
        c.innerHTML=h;
    ...
    UIManager.prototype.trackRow = function(t,i) {
        ...
        return'<div class="track-row" data-track=\''+this.esc(JSON.stringify(t))+'\'><span class="track-row-index">'+(i+1)+'</span><div class="track-row-info"><div class="track-row-cover-placeholder"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="16" height="16"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg></div><div class="track-row-text"><div class="track-row-title">'+this.esc(t.title)+'</div><div class="track-row-artist">'+this.esc(t.artist)+'</div></div></div><span class="track-row-source">'+this.esc(src)+'</span><span class="track-row-duration">'+d+'</span></div>';
    };
    ...
    UIManager.prototype.bindRows = function(c) { var s=this,r=c.querySelectorAll('.track-row'); for(var i=0;i<r.length;i++){(function(row){row.addEventListener('click',function(){try{var t=JSON.parse(row.getAttribute('data-track'));player.play(t);queueManager.add(t);}catch(e){}});})(r[i]);} };
    ...
    UIManager.prototype.showPlModal = function(id) {
        ...
        library.getPlaylist(id).then(function(pl){if(!pl)return;library.getPlaylistTracks(id).then(function(t){
            ...
            h+='</div><div class="mt-4"><button class="btn btn-secondary btn-sm" id="playAllBtn">Play all</button> <button class="btn btn-danger btn-sm" id="delPlBtn">Delete</button></div>';
            ct.innerHTML=h;o.classList.remove('hidden');
            var cl=function(){o.classList.add('hidden');};
            ct.querySelector('.modal-close').addEventListener('click',cl);
            o.addEventListener('click',function(e){if(e.target===o)cl();});
