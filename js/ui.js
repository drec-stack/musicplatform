(function() {
    ...
    UIManager.prototype.showSkeleton = function(container, type) {
        if(type === 'tracks') {
            ...
            container.innerHTML = '<div class="playlist-grid">' + 
                Array(4).fill().map(function() { return `
    ...
    UIManager.prototype.bindTopbar = function() {
        ...
        if(si) si.addEventListener('input',function(){var q=si.value.trim();if(s.st)clearTimeout(s.st);if(q.length>=2){s.st=setTimeout(function(){s.doSearch(q);},350);}});
    };
    ...
    UIManager.prototype.homePage = function() {
        ...
        library.getRecentTracks(8).then(function(t){if(t.length===0)rd.innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg><p>No tracks yet</p><span>Upload some music to get started</span></div>';else{var h='<div class="track-list">';for(var i=0;i<t.length;i++)h+=s.trackRow(t[i],i);h+='</div>';rd.innerHTML=h;s.bindRows(rd);}}).catch(function(){rd.innerHTML='<div class="empty-state"><p>Error loading tracks</p></div>';});
        
        var pd=document.getElementById('homePlaylistsContainer');
        library.getPlaylists().then(function(pl){if(pl.length===0)pd.innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/></svg><p>No playlists</p><span>Create your first playlist</span></div>';else{var h='<div class="playlist-grid">';for(var i=0;i<Math.min(pl.length,6);i++)h+=s.plCard(pl[i]);h+='</div>';pd.innerHTML=h;pd.querySelectorAll('.playlist-card').forEach(function(c){c.addEventListener('click',function(){s.showPlModal(c.getAttribute('data-playlist-id'));});});}}).catch(function(){pd.innerHTML='<div class="empty-state"><p>Error loading playlists</p></div>';});
    ...
    UIManager.prototype.loadLib = function() {
        ...
        if(this.tab==='tracks'){pr=library.getTracks({source:src,sort:srt}).then(function(t){if(t.length===0){ct.innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg><p>No tracks</p><span>Upload or import music</span></div>';return;}var h='<div class="track-list">';for(var i=0;i<t.length;i++)h+=s.trackRow(t[i],i);h+='</div>';ct.innerHTML=h;s.bindRows(ct);});}
        else if(this.tab==='artists'){pr=library.getArtists().then(function(a){if(a.length===0){ct.innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><p>No artists</p></div>';return;}var h='<div class="artist-grid">';for(var i=0;i<a.length;i++)h+='<div class="artist-card"><div class="artist-card-avatar"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="36" height="36"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><div class="artist-card-name">'+s.esc(a[i].name)+'</div><div class="artist-card-info">'+a[i].trackCount+' tracks</div></div>';h+='</div>';ct.innerHTML=h;});}
        else if(this.tab==='albums'){pr=library.getAlbums().then(function(a){if(a.length===0){ct.innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg><p>No albums</p></div>';return;}var h='<div class="album-grid">';for(var i=0;i<a.length;i++)h+='<div class="album-card"><div class="album-card-cover"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="36" height="36"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg></div><div class="album-card-name">'+s.esc(a[i].name)+'</div><div class="album-card-artist">'+s.esc(a[i].artist)+'</div></div>';h+='</div>';ct.innerHTML=h;});}
        else if(this.tab==='favorites') {
            if(window.favorites) {
                pr = window.favorites.getTracks().then(function(t) {
                    if(t.length===0){ct.innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg><p>No favorites</p><span>❤️ Like tracks to see them here</span></div>';return;}
    ...
    UIManager.prototype.doSearch = function(q) {
        ...
        var c=document.getElementById('searchResults'); if(!c)return; c.innerHTML='<div class="loading-container"><div class="loader-spinner"></div><span>Searching...</span></div>';
        library.search(q).then(function(r){if(r.length===0){c.innerHTML='<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><p>Nothing found</p><span>Try a different search term</span></div>';return;}var h='<div class="track-list">';for(var i=0;i<r.length;i++)h+=s.trackRow(r[i],i);h+='</div>';c.innerHTML=h;s.bindRows(c);}).catch(function(){c.innerHTML='<div class="empty-state"><p>Error</p></div>';});
    };
    ...
    UIManager.prototype.doUp = function(f,a,pr) {
        var s=this;a.classList.add('hidden');pr.classList.remove('hidden');pr.innerHTML='<div class="loading-container"><div class="loader-spinner"></div><span>Uploading...</span></div>';
    ...
    UIManager.prototype.plPage = function() {
        ...
        return library.getPlaylists().then(function(pl){
            ...
            if(pl.length===0)h+='<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/></svg><p>No playlists</p><span>Create your first playlist</span></div>';
    ...
    UIManager.prototype.setPage = function() {
        ...
        document.getElementById('clearDataBtn').addEventListener('click',function(){s.confirm('Clear all data?', 'This will delete all tracks, playlists and settings. This cannot be undone!', function(){db.clearAll().then(function(){storage.remove('play_history');storage.remove('queue');localStorage.removeItem('musichub_theme');s.notify('All data cleared','success');s.loadStats();s.go('home');});});});
        return Promise.resolve();
    };
    
    // ========================================
    // TRACK ROW (with favorite icon)
    // ========================================
    ...
    // ========================================
    // BIND ROWS (with context menu & favorites)
    // ========================================
                ...
                row.addEventListener('contextmenu', function(e) {
                    ...
                    try {
                        ...
                        var items = [
                            ...
                            { label: isFav ? '❤️ Remove from favorites' : '♡ Add to favorites', icon: '', action: function() {
                                if(window.favorites) {
                            ...
                            { label: '🗑️ Delete', icon: '', action: function() {
    ...
    UIManager.prototype.showCreatePl = function() {
        ...
        ct.querySelector('.modal-close').addEventListener('click',cl);
        o.addEventListener('click',function(e){if(e.target===o)cl();});
        document.getElementById('savePl').addEventListener('click',function(){var n=document.getElementById('plName').value.trim()||'New Playlist';library.createPlaylist(n,'').then(function(){s.notify('Created','success');cl();s.go('playlists');});});
    };
    ...
    UIManager.prototype.showAddPl = function(tid) {
        ...
        library.getPlaylists().then(function(pl){
            ...
            if(pl.length===0)h+='<p class="text-muted">No playlists yet. Create one first.</p>';
            ...
            ct.querySelector('.modal-close').addEventListener('click',cl);
            o.addEventListener('click',function(e){if(e.target===o)cl();});
            ct.querySelectorAll('.playlist-option').forEach(function(opt){opt.addEventListener('click',function(){library.addToPlaylist(opt.getAttribute('data-id'),tid).then(function(){s.notify('Added to playlist','success');cl();});});});
    ...
    UIManager.prototype.confirm = function(t,m,cb) {
        ...
        ct.innerHTML='<div class="modal-header"><h3 class="modal-title">'+t+'</h3><span class="modal-close">×</span></div><p class="mb-4" style="color:var(--text-secondary);">'+m+'</p><div style="display:flex;gap:8px;"><button class="btn btn-secondary" id="cancelBtn">Cancel</button><button class="btn btn-danger" id="confirmBtn">Confirm</button></div>';
    ...
    UIManager.prototype.updatePlayerUI = function() { 
        ...
        if(i)i.innerHTML=player.playing?'<rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/>':'<polygon points="5 3 19 12 5 21 5 3"/>'; 
    };
    ...
    UIManager.prototype.loadStats = function() { 
        ...
        library.getStats().then(function(st){
            ...
            if(e)e.innerHTML='<div class="sidebar-stat"><span class="stat-value">'+st.totalTracks+'</span><span class="stat-label">tracks</span></div><div class="sidebar-stat"><span class="stat-value">'+st.totalPlaylists+'</span><span class="stat-label">playlists</span></div><div class="sidebar-stat"><span class="stat-value">'+s.fmtSize(st.storageUsage)+'</span><span class="stat-label">used</span></div>';
            var favCount = window.favorites ? window.favorites.getAll().length : 0;
            if(e) e.innerHTML += '<div class="sidebar-stat"><span class="stat-value">'+favCount+'</span><span class="stat-label">favorites</span></div>';
        }).catch(function(){}); 
    };
    ...
    UIManager.prototype.checkSpotify = function() { 
        if(services.checkSpotifyCallback()){this.notify('Spotify connected','success');this.loadStats();} 
    };
    
    UIManager.prototype.exportLib = function() { 
        var s=this; 
        library.exportLibrary().then(function(d){
            var b=new Blob([d],{type:'application/json'}),u=URL.createObjectURL(b),a=document.createElement('a');
            a.href=u;a.download='musichub-'+new Date().toISOString().split('T')[0]+'.json';
            document.body.appendChild(a);a.click();document.body.removeChild(a);
            URL.revokeObjectURL(u);s.notify('Library exported','success');
        }).catch(function(){s.notify('Export failed','error');}); 
    };
    
    UIManager.prototype.notify = function(m,t) { 
        var c=document.getElementById('notificationContainer');if(!c)return;
        var e=document.createElement('div');e.className='notification '+(t||'info');
        var icon = t==='success' ? '✅ ' : (t==='error' ? '❌ ' : 'ℹ️ ');
        e.textContent = icon + m;
        c.appendChild(e);
        setTimeout(function(){
            e.style.opacity='0';e.style.transform='translateX(100%)';e.style.transition='all 0.3s ease';
            setTimeout(function(){if(e.parentNode)e.parentNode.removeChild(e);},300);
        },3000);
    };
    
    UIManager.prototype.esc = function(s) { 
        if(!s)return'';
        var d=document.createElement('div');
        if(d.textContent!==undefined)d.textContent=s;
        else d.innerText=s;
        return d.innerHTML; 
    };
